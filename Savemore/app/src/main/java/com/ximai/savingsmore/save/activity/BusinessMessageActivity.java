...
public class BusinessMessageActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void cancelBusiness(String id) {
        WebRequestHelper.json_post(BusinessMessageActivity.this, URLText.CANCEL_COLLECT_BUSINESS, RequestParamsPool.cancelBusiness(id, null), new MyAsyncHttpResponseHandler(BusinessMessageActivity.this) {
            @Override
            public void onResponse(int statusCode, Header[] headers, byte[] responseBody) {
                String result = new String(responseBody);
                try {
