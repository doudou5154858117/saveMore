...
public class TakeMeActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null != goodDetial) {
            ...
            phone_number.setText(goodDetial.User.PhoneNumber);
            if (null != goodDetial.Area) {
                location.setText(goodDetial.Province.Name + goodDetial.City.Name + goodDetial.Area.Name + goodDetial.Address);
            } else {
                location.setText(goodDetial.Province.Name + goodDetial.City.Name + goodDetial.Address);
            }
            SimpleDateFormat formatter = new SimpleDateFormat("HH:mm");
            Date curDate = new Date(System.currentTimeMillis());//获取当前时间
            String str = formatter.format(curDate);
            int a = Integer.parseInt(str.split(":")[0]);
            if (a > Integer.parseInt(goodDetial.User.UserExtInfo.StartHours.split(":")[0]) && a < Integer.parseInt(goodDetial.User.UserExtInfo.EndHours.split(":")[0])) {
                is_yingye.setText("营业中");
            } else {
                is_yingye.setText("暂停营业");
            }
            yingye_time.setText(goodDetial.User.UserExtInfo.StartHours + "-" + goodDetial.User.UserExtInfo.EndHours);
            mubiao_location.setText(goodDetial.Address);
        }
        loc_end = new Location(Double.parseDouble(goodDetial.Latitude), Double.parseDouble(goodDetial.Longitude));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.phone:
                if (null != goodDetial.User.PhoneNumber) {
